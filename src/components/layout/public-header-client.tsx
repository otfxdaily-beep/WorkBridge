...
type AuthState = { loggedIn: false } | { loggedIn: true; dashboardHref: string };

export function PublicHeaderClient({ auth }: { auth: AuthState }) {
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/85 backdrop-blur-sm">
      <Container className="flex h-16 items-center justify-between">
        ...
        <div className="hidden lg:flex lg:items-center lg:gap-2">
          {auth.loggedIn ? (
            <>
              <ButtonLink href={auth.dashboardHref} variant="ghost" size="sm">
                Dashboard
              </ButtonLink>
        ...
        <div className="border-t border-slate-200 bg-white lg:hidden">
          <Container className="flex flex-col gap-1 py-4">
            ...
            <div className="mt-2 flex flex-col gap-2 border-t border-slate-100 pt-4">
              {auth.loggedIn ? (
                <>
                  <ButtonLink href={auth.dashboardHref} variant="secondary" onClick={() => setOpen(false)}>
                    Dashboard
                  </ButtonLink>
                  <form action={logoutAction}>
